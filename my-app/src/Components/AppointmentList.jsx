...
const AppointmentList = () => {
  ...
  const fetchAppointments = async () => {
    try {
      const response = await axios.get('https://hospital-appointment-system-backend.onrender.com/appointments');
  ...
  const addAppointment = async (e) => {
    ...
    try {
      if (!editingAppointmentId) {
        ...
        await axios.get('https://hospital-appointment-system-backend.onrender.com/appointments', { name, date, age, city });
      } else {
        
        await axios.put(`https://hospital-appointment-system-backend.onrender.com/appointments/${editingAppointmentId}`, { name, date, age, city });
  ...
  const deleteAppointment = async (id) => {
    try {
      await axios.delete(`https://hospital-appointment-system-backend.onrender.com/appointments/${id}`);
      fetchAppointments();
    } catch (error) {
     ...
     <div className='table-responsive small max-width'>
        ...
        <table className="table  table-sm table-bordered rounded">
            ...
            <tr>
              ...
              <th scope="col">Change Appointment</th>
          ...
          {appointments.map((appointment) => (
            <tr>
              ...
              <td><button style={{width:'40'}} type="button" class="btn btn-sm btn-secondary " onClick={() => handleEditAppointment(appointment)} >Change</button></td>
              <td><button style={{width:'40'}} type="button" class="btn btn-sm btn-secondary " onClick={() => deleteAppointment(appointment._id)} >Cancel</button></td>
            </tr>
             ))}
            </tbody>
            </table>
